...
@section('content')
    ...
    <div class="card mt-5">
        ...
        <div class="card-body">
            @if($post->user)
            <h5 class="card-title mb-3">created by: {{$post->user->name}}</h5>
            @endif
            <p class="card-title mb-3">created by: {{$post->user->email}}</p>
            <p class="card-text mb-3">created at: {{$post['created_at']->format('Y-m-d')}}</p>
            <p class="card-text mb-3">updated at: {{$post['updated_at']->format('Y-m-d')}}</p>
        </div>
    </div>
